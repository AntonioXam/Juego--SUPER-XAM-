class scene2 extends Phaser.Scene {
    ...
    create() { 
        ...
        this.player.lives = 3; // Ajustar las vidas a 5
       ...
       this.enemyKillCount = 22; // Ajustar los enemigos a destruir 
    this.enemyKillCountText = this.add.text(1016, 16, 'Enemigos restantes: ' + this.enemyKillCount, { fontSize: '48px', fill: '#ffff00' }); // Cambiar color a amarillo
        ...
        //cada 10 segundos se añade 1 enemigo que es más grande y más rápido que el anterior
        this.time.addEvent({
            delay: 2000,
            callback: () => {
            const x = Phaser.Math.Between(0, 1920);
            const y = Phaser.Math.Between(0, 540);
            // Obtener el tamaño y velocidad del último enemigo añadido
            const lastEnemy = this.enemigos.getChildren().slice(-1)[0];
            const newScale = lastEnemy ? lastEnemy.scale + 0.1 : 0.5;
            const newSpeed = lastEnemy ? lastEnemy.body.velocity.x + 20 : 100;

            // Añadir un enemigo más grande y más rápido
            const newEnemigo = this.physics.add.sprite(x, y, 'enemigo');
            newEnemigo.anims.play('enemigo', true);
            
            newEnemigo.setBounce(2);
            ...
            // Mover el nuevo enemigo hacia el jugador con la nueva velocidad
            this.physics.moveToObject(newEnemigo, this.player, newSpeed);
            },
            loop: true
        });
    ...
    win() {
        ...
        var button = this.add.text(600, 700, 'Boss Final!', { fontSize: '72px', fill: '#ffffff' });
