class BossScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.spritesheet('bowser_vencido', 'assets/bowser_vencido.png', { frameWidth: 109.5, frameHeight: 121 });
        ...
        this.load.audio('bowserjump', 'assets/bowser_1.mp3');
        this.load.audio('bowser_hit', 'assets/bowser_hit.mp3');
    }
    ...
    create() {
        ...
        // Definir cursores para el movimiento del jugador
        ...
        // Crear plataformas (suponiendo que 'plataformas' es un grupo estático)
        ...
        this.player.setOffset(40, 20); // Ajustar tamaño del jugador
        this.player.lives = 3; // Ajustar las vidas a 3
        ...
        this.municion = 10;
        this.municionText = this.add.text(16, 16, 'Municion: ' + this.municion, { fontSize: '32px', fill: '#00F' });
        ...
        this.input.on('pointerdown', function (pointer) {
            ...
            if (bullet) {
                ...
                this.municion -= 1;
                // Actualizar la munición, darle color y sombra
                ...
                // Voltear el jugador según la dirección del disparo
        ...
        this.time.addEvent({
         ...
         //marcador de vidas
            this.vidasText = this.add.text(16, 50, 'Vidas: ' + this.player.lives, { fontSize: '32px', fill: '#F00' });
    ...
    update() {
        ...
        this.playerSpeed = 200; // Velocidad del jugador
    ...
    createBowser() {
        this.bowser = this.physics.add.sprite(1920 - 16, 800, 'bowser'); // posicionar a bowser en la esquina superior derecha
        ...
        this.bowser.lives = 20; // Ajustar las vidas de Bowser a 20
        ...
        this.bowser.flipX = true; // Mirar hacia la izquierda
    }
    ...
    moveBowser() {
        const moveDuration = 1000; // duración del movimiento de Bowser
        const jumpHeight = -1900; // altura del salto de Bowser
        // tardar menos en caer
        this.bowser.body.gravity.y = 1000;
    
        // Mover a Bowser hacia el jugador cada 2 segundos
        this.time.addEvent({
            delay: moveDuration,
            callback: () => {
                //  movimiento de bowser hacia el jugador con un 80% de probabilidad
                if (Math.random() < 0.8) {
                ...
                // voltear a Bowser según la dirección del movimiento
                if (this.bowser.body.velocity.x > 0) {
                    this.bowser.flipX = true; // Mirar hacia la derecha
                } else if (this.bowser.body.velocity.x < 0) {
                    this.bowser.flipX = false; // Mirar hacia la izquierda
                }

                // Reproducir la animación de movimiento de Bowser
                this.bowser.anims.play('bowser', true);
            },
            loop: true
        });

        // Hacer que Bowser salte frecuentemente
        this.time.addEvent({
            delay: moveDuration, // Ajustar el retraso según sea necesario
            callback: () => {
                if (this.bowser.body.touching.down) {
                    this.bowser.setVelocityY(jumpHeight);
                    // Reproducir la animación de salto de Bowser
                    this.bowser.anims.play('bowser', true);
                    //sonido de salto
                    this.sound.add('bowserjump').play();
    ...
    playerHit(player, fireball) {
        if (!player.isInvencible) {
            ...
            player.setTint(0xff0000); // Opcional: Añadir un efecto visual para indicar daño
            ...
            this.vidasText.setText('Vidas: ' + player.lives); // Actualizar el marcador de vidas
            this.sound.add('hit').play();
            this.time.delayedCall(1000, () => {
                player.isInvencible = false;
            });
        }
        fireball.disableBody(true, true); // Hacer que la bola de fuego desaparezca
    }

    // daño al jugador por bowser
    ...
    // daño a bowser
    hitBowser(bowser, bullet) {
        bowser.lives -= 1;
        bullet.disableBody(true, true); // Deshabilitar el cuerpo del disparo

        if (bowser.lives <= 0) {
            // Reproducir sonido de nivel ganado
            this.sound.add('nivel_ganado').play();
            bowser.anims.play('bowser_vencido', true); // Reproducir la animación de Bowser vencido
            this.time.delayedCall(1000, () => {
                bowser.disableBody(true, true); // Deshabilitar el cuerpo de Bowser después de la animación
            });
            // Desaparecer jugador
            ...
            bowser.setTint(0xff0000); // Añadir un efecto visual para indicar daño
            this.time.delayedCall(100, () => {
                bowser.clearTint();
                this.sound.add('bowser_hit').play();
            });
        }
    }
